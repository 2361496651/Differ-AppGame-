...
- (UIButton *)completeBtn
{
    if (_completeBtn == nil) {
        ...
        NSString *btnTitle = [NSString stringWithFormat:@"完成(%ld)",self.selectedImageIndex.count];
...
- (void)setRightBarButtonWithTitle:(NSString *)title
{
    ...
    if ([self.selectedImageIndex containsObject:title]) {// 这张照片已经被选中
        NSString *total = [NSString stringWithFormat:@"%ld",self.selectedImageIndex.count];
...
- (void)rightBtnClick
{
    ...
    NSString *visibleIndexStr = [NSString stringWithFormat:@"%ld",visibleIndex.row];
...
- (void)viewDidLoad {
    ...
    self.title = [NSString stringWithFormat:@"%ld/%ld",self.indexPath.row+1,self.picALAssets.count];
    
    [self setRightBarButtonWithTitle:[NSString stringWithFormat:@"%ld",self.indexPath.row]];
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    NSIndexPath *visibleIndex = [self.collectionView indexPathsForVisibleItems].firstObject;
    self.title = [NSString stringWithFormat:@"%ld/%ld",visibleIndex.row+1,self.picALAssets.count];
    
    [self setRightBarButtonWithTitle:[NSString stringWithFormat:@"%ld",visibleIndex.row]];
