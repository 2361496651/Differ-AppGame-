...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    NSString *title = [NSString stringWithFormat:@"完成(%ld/9)",self.imageIndex.count];
...
//    if (self.imageIndex.count < 9) {
    ...
    [self.imageIndex addObject:[NSString stringWithFormat:@"%ld",index.row]];
...
- (void)collectionViewWithDeSelected:(ZCJCollectionViewCell *)collectionViewCell
{
    ...
    // 从数组里删除这个索引
    [self.imageIndex removeObject:[NSString stringWithFormat:@"%ld",index.row]];
